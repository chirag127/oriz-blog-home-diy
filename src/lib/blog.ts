...
/** Map a raw category string → one of the five workbench desks. */
export function verticalFor(category: string): Vertical {
  const c = category.toLowerCase()
  if (/(outdoor|garden|yard|patio|balcony|plant|deck)/.test(c)) return 'health'
  if (/(repair|fix|maintenance|plumb|electric|patch|leak|tool)/.test(c)) return 'finance'
  if (/(organiz|organis|declutter|storage|space|closet|kitchen|shelf|small-space)/.test(c))
    return 'dev'
  if (/(refresh|decor|interior|paint|style|budget|room|makeover|design)/.test(c)) return 'culture'
  return 'personal'
}
